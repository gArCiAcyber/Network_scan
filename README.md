...
Amass 3.x hostname output and 4.x graph output are supported; graph parsing is tested against the 4.2.0 format. Amass 5.x remains unsupported: versions that retain the required legacy flags warn before their provider run reports the unsupported engine/session lifecycle; missing required flags stop startup. Use `--amass-path` to select a compatible executable. Discovery candidates must be valid DNS hostnames within the requested domain before reaching DNSx.
