...
Local version/help checks reject unsupported versions and missing CLI options before enumeration. Untested stable versions within supported majors produce a warning; JSON provider entries record the observed version and compatibility status. The packaged registry and daily release-monitoring workflow are described in [External provider compatibility](docs/provider_compatibility.md), including tested baselines, test coverage, and the update review process.
