...
def run_port_scan(
    ...
    display = TCPScanDisplay(target, len(ports_to_scan))
    ...
    result = scan_tcp_ports(
        ...
        progress_callback=display.handle_progress,
        open_port_callback=display.handle_open_port,
        service_probe_start_callback=display.start_service_probe,
        service_probe_complete_callback=display.complete_service_probe,
    )
    clear_dynamic_line()
    return result
...
def run_passive_subdomain_discovery(
    ...
    display = PassiveDiscoveryDisplay(domain)
